...
Get_beta_cov_UV <- function(sumstats, MAF, zeta.ret, R.C, KA, MAF.thres = 0.000588,
                            trait.list,
                            main.thres = 5e-3, test = "GEI"){
  D <- length(sumstats)
  K <- length(sumstats[[1]])
  m <- length(MAF)
  snp.list <- names(MAF)
  ...
  # print(main.p)
  system.time(variant.p <- variantP3(sumstats = sumstats, zeta.ret = zeta.ret,
  ...
  highLD <- try(findCorrelation(R.C[colnames(R.C) %in% names(MAC10), colnames(R.C) %in% names(MAC10)],
    ...
    for(d in 1:D) {
      ...
      for(trait.pair in 1:nrow(trait.grid)) {
        ...
        if(k2 <= k1) {
          ...
          part1 <- t(beta.cov.tmp[[which(trait.grid$trait1 == k2 & trait.grid$trait2 == k1)]])
        }
        beta.cov.tmp[[trait.pair]] <- part1
